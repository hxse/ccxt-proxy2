...
- **日志重建 (`rebuild_log_from_data`)**:
    - 验证从数据文件重建日志的功能。
    - **按首尾衔接重建**: 仅当文件区间首尾衔接/重叠时合并为同一段。
    - **断裂拆段**: 文件区间存在断裂时，重建日志应生成多段而非单段。
...
- **分区顺序读取读够即停**: 从 `start_time` 命中起始分区后，顺序读取后续分区，满足 `count` 立即停止。
- **+1 条件补偿**:
...
- **日志损坏自动重建**: 验证损坏的日志文件会触发警告并自动重建。
- **去重后无新增仍保存**: 验证即使没有新K线，已有K线的更新也会被保存。
- **日志语义不混合**: 验证日志不包含纯缓存复用段，仅记录实际落盘增量。
