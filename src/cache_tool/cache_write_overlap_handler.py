...
def adjust_write_range_and_delete_overlapped(
    ...
    for f in sorted_cache_files:
        ...
        _new_data_start = format_timestamp(
            convert_ms_timestamp_to_utc_datetime(new_data_start)
        )
        _new_data_end = format_timestamp(
            convert_ms_timestamp_to_utc_datetime(new_data_end)
        )
        _old_data_start = format_timestamp(
            convert_ms_timestamp_to_utc_datetime(old_data_start)
        )
        _old_data_end = format_timestamp(
            convert_ms_timestamp_to_utc_datetime(old_data_end)
        )

        # 1. 新数据完全在旧数据内部（完全被缓存） -> 无需写入，直接返回
        if new_data_start >= old_data_start and new_data_end <= old_data_end:
            print(
                f"✅ 新数据 ({_new_data_start}-{_new_data_end}) 已被缓存文件 ({_old_data_start}-{_old_data_end}) 完全覆盖，无需写入。"
            )
        ...
        # 2. 新数据完全覆盖旧数据 -> 标记旧文件为删除
            ...
            print(
                f"🔄 新数据 ({_new_data_start}-{_new_data_end}) 完全覆盖旧缓存文件 ({_old_data_start}-{_old_data_end})，标记为删除。"
            )
        ...
        # 3. 新数据在旧数据之前且有重叠 -> 调整写入的结束时间，使其不包含重叠部分
            ...
            _end_time_to_write = format_timestamp(
                convert_ms_timestamp_to_utc_datetime(end_time_to_write)
            )
            print(
                f"⚠️ 新数据 ({_new_data_start}-{_new_data_end}) 与旧缓存 ({_old_data_start}-{_old_data_end}) 重叠，调整写入结束时间 {_end_time_to_write}"
            )
        ...
        # 4. 新数据在旧数据之后且有重叠 -> 调整写入的起始时间，使其不包含重叠部分
            ...
            _start_time_to_write = format_timestamp(
                convert_ms_timestamp_to_utc_datetime(start_time_to_write)
            )
            print(
