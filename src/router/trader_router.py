...
CCXT_READ_RESPONSES: dict[int | str, dict[str, Any]] = {
    ...
    422: {"description": "参数、symbol、timeframe、variant 或 Provider capability 无效。"},
...
@ccxt_router.get(
    "/fetch_ohlcv/since-limit", response_model=OhlcvResponse,
...
@ccxt_router.get(
    "/fetch_ohlcv/since-latest", response_model=OhlcvResponse,
...
@ccxt_router.get(
    "/fetch_ohlcv/latest-limit", response_model=OhlcvResponse,
...
@ccxt_router.get(
    "/fetch_balance", response_model=BalanceResponse, summary="获取账户余额",
    description="只读调用指定 CCXT identity 的 unified balance API，返回 free/used/total 以及 Provider 扩展字段。",
    response_description="CCXT unified balance。", responses=CCXT_READ_RESPONSES,
)
...
@ccxt_router.get(
    "/fetch_tickers", response_model=TickersResponse, summary="批量获取行情报价",
    description="只读获取逗号分隔 symbols 的 CCXT unified tickers；不传 symbols 时由 Provider 决定返回范围。",
    response_description="以 canonical symbol 为 key 的 ticker 字典。", responses=CCXT_READ_RESPONSES,
)
...
@ccxt_router.get(
    "/fetch_market_info", response_model=MarketInfoResponse,
    summary="获取下单所需市场信息",
    description="返回 linear、settle、amount step、最小数量和 contract size；只有 position 明确提供时 leverage 才为整数，否则为 null。",
    response_description="稳定的市场精度、限额和可确认杠杆信息。", responses=CCXT_READ_RESPONSES,
)
...
@ccxt_router.get(
    "/fetch_order", response_model=OrderResponse, summary="按 ID 查询订单",
    description="只读查询指定订单。Binance Futures normal endpoint 明确不存在时会 fallback 到 conditional/stop endpoint。",
    response_description="CCXT unified order。", responses=CCXT_ORDER_READ_RESPONSES,
)
...
@ccxt_router.post(
    "/create_market_order", response_model=OrderResponse, summary="创建市价单",
    description="有状态操作：按 side/amount 创建 market order；Provider 扩展参数和 clientOrderId 会由 CcxtClient 统一翻译。网络失败不会自动重试。",
    response_description="Provider 接受后返回的 unified order。", responses=CCXT_WRITE_RESPONSES,
)
...
@ccxt_router.post(
    "/create_limit_order", response_model=OrderResponse, summary="创建限价单",
...
@ccxt_router.post(
    "/create_stop_market_order", response_model=OrderResponse,
    summary="创建止损市价触发单",
    description="有状态操作：将 triggerPrice 翻译为 Provider stop-loss trigger；默认 reduceOnly=true，网络失败不会自动重试。",
    response_description="创建后的 unified conditional order。", responses=CCXT_WRITE_RESPONSES,
)
...
@ccxt_router.post(
    "/create_take_profit_market_order", response_model=OrderResponse,
    summary="创建止盈市价触发单",
...
@ccxt_router.post(
    "/close_position", response_model=ClosePositionResponse, summary="按市价平仓",
    description="有状态操作：读取当前 positions，并对指定 symbol/可选 side 的非零仓位提交 reduceOnly market order；不是原子操作。",
    response_description="提交平仓后再次查询得到的剩余 positions。", responses=CCXT_WRITE_RESPONSES,
)
...
@ccxt_router.post(
    "/cancel_all_orders", response_model=CancelAllOrdersResponse,
    summary="撤销全部挂单",
    description="高影响有状态操作：撤销指定 symbol 或 Provider 范围内的全部订单；Binance Futures 同时处理 normal 与 conditional orders。",
    response_description="Provider 返回的撤单结果。", responses=CCXT_WRITE_RESPONSES,
)
...
@ccxt_router.get(
    "/fetch_open_orders", response_model=OrdersResponse, summary="查询当前挂单",
    description="只读查询 open orders；Binance Futures 会合并 normal 与 conditional 结果并按 order ID 去重。",
    response_description="按时间倒序的 unified open orders。", responses=CCXT_READ_RESPONSES,
)
...
@ccxt_router.get(
    "/fetch_closed_orders", response_model=OrdersResponse, summary="查询历史已关闭订单",
    description="只读查询 closed/canceled order history；Binance Futures 会合并 normal 与 conditional 结果。",
    response_description="Unified closed order 列表。", responses=CCXT_READ_RESPONSES,
)
...
@ccxt_router.get(
    "/fetch_my_trades", response_model=TradesResponse, summary="查询账户成交历史",
    description="只读查询当前账户的 CCXT unified trades，可按 symbol、since 和 limit 缩小范围。",
    response_description="Unified trade 列表。", responses=CCXT_READ_RESPONSES,
)
...
@ccxt_router.get(
    "/fetch_positions", response_model=PositionsResponse, summary="查询合约持仓",
    description="只读查询全部或指定 symbols 的 unified positions；symbol 会先通过 market-scope capability 校验。",
    response_description="Unified position 列表。", responses=CCXT_READ_RESPONSES,
)
...
@ccxt_router.post(
    "/set_leverage", response_model=GenericResponse, summary="设置账户杠杆",
    description="有状态操作：设置指定 symbol/Provider scope 的 leverage；网络失败不会自动重试，Provider 冲突返回稳定错误。",
    response_description="Provider 的杠杆设置结果。", responses=CCXT_WRITE_RESPONSES,
)
...
@ccxt_router.post(
    "/set_margin_mode", response_model=GenericResponse, summary="设置保证金模式",
    description="有状态操作：把 marginMode 设置为 cross 或 isolated；已有持仓/当前状态冲突由 Provider 返回并映射为稳定错误。",
    response_description="Provider 的保证金模式设置结果。", responses=CCXT_WRITE_RESPONSES,
)
...
@ccxt_router.post(
    "/cancel_order", response_model=OrderResponse, summary="按 ID 撤销订单",
    description="有状态操作：撤销指定订单。Binance Futures normal endpoint 明确不存在时会 fallback 到 conditional endpoint；网络失败不重试。",
    response_description="撤销后的 unified order。", responses=CCXT_WRITE_RESPONSES,
)
