...
> **历史决策记录。** 本文保留当时的方案比较；当前行为与约束以 [current spec](../../current_specs/architecture.md) 为准。
...
方案理论通用，但对当前实际请求成本过高。目标方案只在请求起点选一个最佳 prefix，进入 network 后不再读第二个 segment。详细历史保留在 [已否决 Resolver](fragmented_resolver.md) 和 [已否决 Estimator](median_estimator.md)。
...
1. [目标架构](../../current_specs/architecture.md)
2. [OHLCV Route contract](../../current_specs/ccxt_ohlcv.md)
3. [DuckDB cache 模型](../../current_specs/ohlcv_cache_storage.md)
