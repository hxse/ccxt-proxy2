...
> **状态：Research snapshot。** 行为结论来自当时的 CCXT sandbox/实盘经验；它记录 Provider 差异，不承诺交易所永远保持相同行为。目标重构必须把仍然有效的差异收口到 [`CcxtClient`](../../current_specs/ccxt_client.md)，不得继续散落在 Route 或多个 adapter 中。
