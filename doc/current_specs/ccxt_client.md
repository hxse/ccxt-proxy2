...
一次多页 OHLCV 操作不整体持锁，retry backoff 也不持锁。这使 order call 可以在 OHLCV pages 之间执行。当前实现不做请求优先级队列。
...
`variant` 覆盖 mark/index/premium-index 等价格系列。任何未编入 identity 的影响数据内容参数都会导致 cache 串数据，必须列入数据身份编码。
...
研究出的 Binance/Kraken 订单差异仍然保留，只是实现位置收口到 Client，见 [订单行为对比](../archive/ccxt_research/order_behavior_comparison.md)。
